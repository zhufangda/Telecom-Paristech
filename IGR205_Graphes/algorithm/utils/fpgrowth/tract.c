...
void findFItemset(FILE *file, ItemLink *tracts, NodeLink *np, int support,
		int transNum, int itemsNum) { /* find frequent itemset */
	...
	if (trans)
		free(trans); /* free transaction buffer */
}

void createFItemset(ItemLink *tracts, int idx, NodeLink *np, int support,
		char *trans, int itemsNum) { /* create freq. itemset */
...
void createTract(ItemLink *tracts, int idx, char *field) { /* put the idxth transaction into tracts array*/
...
void deleteUnsupport(ItemLink *tracts, int length, int support,
		NodeLink *hashTable, int size) { /* delete the items, which does not reach the
...
void sortEveryTract(ItemLink *tracts, int length, NodeLink *hashTable, int size,
		int support) { /* sort all of the transactions with select sort */
	int i;
	for (i = 0; i < length; ++i) {
		selectSort(&tracts[i], hashTable, size, support);
	}
}
void selectSort(ItemLink *head, NodeLink *hashTable, int size, int support) { /* descending sort transaction based on support,
...
int hash(char *buf, int tableSize) { /* BKDR Hash Function */
...
void createHashTable(NodeLink *np, char *field, int support, int tableSize) { /* use link address method to create hash table */
	...
	for (p = np[h]; p; p = p->next) { /* traverse the whole hash table */
		if (strcmp(p->field, field) == 0) { /* if here it is */
			...
			if (p->support >= support)
				p->exceeded = 'Y';/* exceed the threshold */
			return; /* and return */
		}
	}
	...
	p->support = 1;
	if (support > 1)
		p->exceeded = 'N'; /* default value when initializing */
	else
		p->exceeded = 'Y';
...
int getNumOfExceeded(NodeLink *hashTable, int size) { /* get the number of items whose support has exceeded the threshold */
	int i;
	int num = 0;
	for (i = 0; i < size; ++i) {
