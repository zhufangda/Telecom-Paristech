...
extern void createHeadTable(NodeLink *hashTable, int size,
		FpHeadTable *headTable, int numExceeded);
extern void _sortHeadTable(FpHeadTable *headTable);
extern void createFpTree(FpTree *fptree, FpHeadTable *headTable,
		ItemLink *tracts, int size);
static void _buildCondFpTree(FpTree *cfptree, FpTree *fptree, int idx,
		int support);
