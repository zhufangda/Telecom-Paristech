...
void createFpTree(FpTree *fptree, FpHeadTable *headTable, ItemLink *tracts,
		int size) {
	int i;
...
FpNode* _matchedCfp(FpNode *q, Pattern p) { /* if matching, node's support++ and
 return the matched child(true), otherwise NULL(false) */
	for (; q; q = q->sibling) { /* when using large data set,
	 ...
	 I don't know why */
		if ((q->field != NULL) && (p->field != NULL)
				&& (strcmp(q->field, p->field) == 0)) {
...
void _insertCfpTree(Pattern pt, FpNode *node, FpHeadTable *headTable) {
	...
	if ((child != NULL) && (child = _matchedCfp(child, pt)))
		;/*if node has first child and match,
...
FpNode* _matched(FpNode *p, ItemLink itemLink) { /* if matching, node's support++ and
 return the matched child(true), otherwise NULL(false) */
	for (; p; p = p->sibling) {
...
void _insertTree(ItemLink itemLink, FpNode **node, FpHeadTable *headTable) {
	int i;
	if (!itemLink || itemLink->field == NULL)
		return;
	if ((*node) == NULL) {
		printf("error parameter, node is null.\n");
		return;
	...
	FpNode *child = (*node)->eldest;
	if ((child != NULL) && (child = _matched(child, itemLink)))
		;/* if node has first child and
...
void _linkSameNode(FpNode **node, FpHeadTable *headTable) { /* link the same item-name nodes with item head table */
	int i;
...
void createHeadTable(NodeLink *ht, int size, FpHeadTable *headTable,
		int numExceeded) { /* create a item head table for sorting and projecting */
	...
	for (i = 0; i < size; ++i) { /* for every hash value */
		...
		for (; p; p = p->next) { /* and for every node */
			if (p->exceeded == 'N')
				continue; /* if not exceeded, do not put into head table, */
...
void _sortHeadTable(FpHeadTable *headTable) { /* binary insert sort */
	int i, j;
	int low, high, m;
...
void showFpTree(FpTree *fptree) {
	...
	for (i = 0; i < c; ++i) {
		...
		while (p) {
			printf(
					"field=[%s:%d],parentfield=[%s:%d],eldest=[%s:%d],sibling=[%s:%d],same=[%s:%d]\n",
					p->field, p->support, p->parent->field, p->parent->support,
					p->eldest == NULL ? "-" : p->eldest->field,
					p->eldest == NULL ? 0 : p->eldest->support,
					p->sibling == NULL ? "-" : p->sibling->field,
					p->sibling == NULL ? 0 : p->sibling->support,
					p->same == NULL ? "-" : p->same->field,
					p->same == NULL ? 0 : p->same->support);
			p = p->same;
		}
	}
}
...
int _singlePath(FpTree *fptree) {
	...
	while (p) {
		if (p->eldest && p->eldest->sibling)
			return 0;/* p has at least two children */
		p = p->eldest;
	}
	return 1;
}
...
void fpgrowth(FpTree *fptree, FpNode *a, int support, FILE *fp) {
	...
	if (_singlePath(fptree)) {
		...
		for (i = ht->count - 1; i >= 0; --i) {
			...
			if (!cfptree) {
				printf(
						"*** out of memory! *** failed to create conditional fptree \n");
				exit(-1);
			}
...
void _buildCondFpTree(FpTree *cfptree, FpTree *fptree, int idx, int support) {
	...
	if (NULL
			== (cfptree->headTable = (FpHeadTable *) malloc(sizeof(FpHeadTable)))) {
...
void _buildHeadTable(FpHeadTable *ht, PatternBase *pbh, int support) {
	...
	if (!listHead) {
		perror("can not create link list head ");
		exit(-1);
	}
	...
	if (ht->lists == NULL) {
		perror("can not create head table lists ");
		exit(-1);
	}
	for (p = listHead->next; p; p = p->next) { /* I'm NOT sure whether the link list is descend order or not */
		if (p->exceeded == 'N')
			continue; /* so, traverse it then sort the head table again, ignoring the unsupport one */
		FpListLink q = (FpListLink) malloc(sizeof(FpList));/* else create a node head */
		if (!q) {
			perror("out of memory!!!");
			exit(-1);
...
int _assistUtil(NodeLink *h, PatternBase pbh, int support) {/* build a assistant list for building the head table,
...
void _prunePatternBase(PatternBase pbh, NodeLink list, int n) { /* delete n unsupport node(s) in the p.b. */
	Item items[n];
	...
	for (q = q->eldest; q; q = q->eldest) {
		...
		while (p) {
			printf("field=[%s:%d],sibling=[%s],eldest=[%s]\n", p->field,
					p->support, p->sibling == NULL ? "-" : p->sibling->field,
					p->eldest == NULL ? "-" : p->eldest->field);
			p = p->sibling;
		}
	}
...
void _buildCondPatternBase(PatternBase *pbHead, FpTree *fptree, int idx) {/* use FpNode to create nodes of pattern bases,
...
void _generatePatterns(FpNode *a, FpTree *fptree, FILE *fp) {
	...
	while (n--) { /* traverse whole path which has at least one node */
		for (i = 0, b = cur->eldest; i <= n; ++i) {/* for each combination (denoted as b) of nodes in the path  */
			FpNode *b_cur = b;
			for (support = MAX_SUPPORT, j = 0; j <= i && b_cur; ++j, b_cur =
					b_cur->eldest) {/* traverse one combination */
