...
int main(int argc, char *argv[]) {
	...
	if (argc >= 9) {
		...
		printf(
				"usage: %s -i inFile -o outFile -t support -n transNum -m itemsNum\n",
				PRGNAME);
		...
		if ((*s == '-') && *++s) { /* if argument is an option */
			while (*s) { /* traverse the options */
				...
				case 'i':
					...
					break;
				case 't':
					support = strtod(argv[++i], NULL);
					break;
				case 'n':
					transNum = atoi(argv[++i]);
					break;
				case 'm':
					itemsNum = atoi(argv[++i]);
					break;
				case 'o':
					outFile = argv[++i];
					break;
	...
	support = ceil((support >= 0) ? 0.01 * support * transNum : -support);/* compute absolute support value */
	if (inFile && *inFile) {
		in = fopen(inFile, "r");
	} else {
		inFile = "<stdin>";
